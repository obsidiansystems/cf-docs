...
Scan provides a couple shortcuts for using the [scan_bulk_data_api](/sdks-tools/api-reference/splice-scan-bulk-data-api) to look up Amulet and Amulet Name Service records.

## Amulet Summaries

While it's possible to figure total Amulet balance from the [scan_bulk_data_api](/sdks-tools/api-reference/splice-scan-bulk-data-api), figuring the effect of holding fees can be tricky for a new Splice app unfamiliar with the on-ledger data model of Amulet.

With a valid time from [v0_state_acs_snapshot-timestamp](/sdks-tools/api-reference/splice-scan-bulk-data-api), knowing the latest migration ID, and one or more party IDs to look up Amulet holdings for, Scan will gather Amulet statistics for each matching party and yield them. Here's an example payload passed to /v0/holdings/summary on a test network:
...
This response is in the same format as [v0_state_acs](/sdks-tools/api-reference/splice-scan-bulk-data-api) but is restricted to contracts of Daml templates `Amulet` and `LockedAmulet`.
