...
| API                                  | Purpose                                            |
|--------------------------------------|----------------------------------------------------|
| [validator-api-user-wallet](/sdks-tools/api-reference/splice-validator-api)          | Users interacting with their wallets               |
| [validator-api-user-wallet-internal](/sdks-tools/api-reference/splice-validator-api) | Internal components interacting with user wallets  |
| [validator-api-external-signing](/sdks-tools/api-reference/splice-validator-api)     | External signing for `Canton Coin`                 |
| [validator-api-user-management](/sdks-tools/api-reference/splice-validator-api)      | Managing users hosted by the (super)validator node |
| [validator-api-internal](/sdks-tools/api-reference/splice-validator-api)             | Operators managing the (super)validator node       |
| [validator-api-ans](/sdks-tools/api-reference/splice-validator-api)                  | Used for the Amulet Name Service                   |
| [validator-api-scan-proxy](/sdks-tools/api-reference/splice-validator-api)           | BFT proxy to the public scan API                   |

Please see [app_dev_openapi_conventions](/sdks-tools/api-reference/splice-http-apis) to learn about the API stability annotations and the contract payload encoding used in the OpenAPI specifications referenced below.

## User wallet API

These endpoints are intended for users to programmatically interact with their wallets.

**Authorization:** Authentication with a JWT token as described in [app-auth](/sdks-tools/api-reference/splice-http-apis), where the subject claim of the token is the user whose wallet the endpoint operates on.
...
<Note>
**Deprecated** (since `splice-0.4.11`): Use the [Canton Network Token Standard APIs](/sdks-tools/api-reference/splice-daml-apis) instead.
</Note>

Splice Wallet [transfer offers](/sdks-tools/api-reference/splice-validator-api) are a legacy two-step workflow to transfer Canton Coin between users. They work as follows:
...
**Authorization:** Authentication with any valid JWT token as described in [app-auth](/sdks-tools/api-reference/splice-http-apis).
...
**Authorization:** Authentication with a JWT token as described in [app-auth](/sdks-tools/api-reference/splice-http-apis), where the subject claim of the token is the validator operator user (for `/v0/admin/users`), or the user onboarding itself (for `/v0/register`).
...
**Authorization:** Authentication with a JWT token as described in [app-auth](/sdks-tools/api-reference/splice-http-apis), where the subject claim of the token is the validator operator user.
...
**Authorization:** Authentication with a JWT token as described in [app-auth](/sdks-tools/api-reference/splice-http-apis), where the subject claim of the token is the user who is requesting the new ANS entry.
...
These endpoints implement a BFT proxy to the public scan API. They have the same interfaces as the equally named endpoints in the public [app_dev_scan_api](/sdks-tools/api-reference/splice-scan-api).
...
**Backwards compatibility:** See the corresponding endpoint in the [app_dev_scan_api](/sdks-tools/api-reference/splice-scan-api).
